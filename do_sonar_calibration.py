...
# Implement reader to find new .nc files when current one stops being updated (in file_listen()).
# Choose beam_group based on beam type rather than assuming the horizontal beam is in beam_group1.
...
#############################################################
numPings = 100 # to show in the echograms and sphere amplitude plots
...
horizontalBeamGroup = 'Sonar/Beam_group1' # should automate this...

# Configure file locations here
from pathlib import Path
# The netCDF files generated by the sonar:
watchDir = Path(r'C:\Users\gavin\Data - not synced\example_data\Furuno FCV30')
# The directory where log files get put:
...
def main():
    ...
    # This line is for use on live data
    t = threading.Thread(target=file_listen, args=(watchDir,))
    # This line is for testing of existing data
...
def file_listen(watchDir):
    ...
    while True: # could add a timeout on this loop...
        ...
        if mostRecentFile == f_previous: # no new file was found
            logging.info('No newer file found. Will try again.')
            ...
            while noNewDataCount <= maxNoNewDataCount:
                ...
                t = f[horizontalBeamGroup + '/ping_time'][-1]
                
                if t > t_previous: # there is a new ping in the file
                    dr = f[horizontalBeamGroup + '/backscatter_r'][-1]
                    
                    x = f[horizontalBeamGroup + '/beam_direction_x'][-1]
                    y = f[horizontalBeamGroup + '/beam_direction_y'][-1]
                    ...
                    samInt = f[horizontalBeamGroup + '/sample_interval'][-1]
                    ...
                    logging.info('No new data found in file {}. Will try again ({} of {} attempts).'
                                 .format(mostRecentFile.name, noNewDataCount, maxNoNewDataCount))
                    noNewDataCount += 1
                f.close()
...
def file_replay(watchDir):
    ...
    t = f[horizontalBeamGroup + '/ping_time']
    
    # Send off each ping at a sedate rate...
    for i in range(0, t.shape[0]):
        #print('ping')
        dr = f[horizontalBeamGroup + '/backscatter_r'][i]
        # take log of dr to make the rest of the code simplier
        for j in range(0,dr.shape[0]):
            dr[j] = np.log10(dr[j])
            
        x = f[horizontalBeamGroup + '/beam_direction_x'][i]
        y = f[horizontalBeamGroup + '/beam_direction_y'][i]
        
        # convert x,y,z direction into a horizontal angle for use elsewhere
        theta = np.arctan2(y, x)
        theta[0] = -theta[0] # first beam is usually 180, but it should be -180.
                
        samInt = f[horizontalBeamGroup + '/sample_interval'][i]
        c = f['Environment/sound_speed_indicative'][()]
        # send the data off to be plotted
